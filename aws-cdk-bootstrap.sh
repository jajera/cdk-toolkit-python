...
create_bootstrap() {
  # Check if CDK is already initialized locally
  if [[ -f "cdk.json" ]]; then
    echo "CDK project is already initialized in this directory. Skipping cdk init."
  else
    echo "Initializing CDK project..."
    cdk init app --language python
    if [[ $? -ne 0 ]]; then
      echo "Error: Failed to initialize the CDK project." >&2
      exit 1
    fi
  fi
...
delete_bootstrap() {
  ...
  # Get the S3 bucket name associated with the stack
  ...
  if [[ -n "$BUCKET_NAME" ]]; then
    ...
    if [[ $? -ne 0 ]]; then
      echo "Error: Failed to delete contents of bucket $BUCKET_NAME." >&2
      exit 1
    fi
    ...
    if [[ $? -ne 0 ]]; then
      echo "Error: Failed to delete bucket $BUCKET_NAME." >&2
      exit 1
    fi
  fi
  ...
  if [[ $? -ne 0 ]]; then
    echo "Error: Failed to delete CloudFormation stack $CUSTOM_STACK_NAME." >&2
    exit 1
  fi
